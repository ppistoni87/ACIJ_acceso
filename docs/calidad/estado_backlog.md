...
| HU-035 | Poblar el corpus con datos reales y conciliar | orquestacion | P0 | EN_CURSO | `scripts/poblar_corpus.sh`<br>`scripts/corrida_limpia.sh`<br>`src/backend_normativo/ingesta/importadores/infoleg.py`<br>`docs/operacion/poblacion_real.md`<br>`docs/reportes/actualizacion_controlada.md`<br>`docs/reportes/corrida_limpia.md`<br>`docs/curaduria`<br>`src/backend_normativo/curacion/beneficios.py`<br>`tests/unit/test_lecturas_curadas.py`<br>`bash scripts/poblar_corpus.sh`<br>`bn ingesta importar-infoleg`<br>`bn calidad ensayo-actualizacion`<br>_Falta: La población entera se corre con un comando: `scripts/poblar_corpus.sh` es la única definición del procedimiento y `scripts/corrida_limpia.sh` lo ejercita sobre una base que se crea vacía. Están el catálogo nacional entero (423.718 normas), el padrón RENABAP (6.467 barrios) y los tres directorios. Doce beneficios están curados desde el texto capturado: cuatro porteños —el apoyo habitacional de la Ley 6935, el Régimen de Becas de la Ley 2917, el subsidio para familias en situación de calle del Decreto 690/06 y la beca de comedor de la Ordenanza 43.478— y ocho de la Ley nacional 24.714, curados por separado porque son prestaciones distintas sobre la misma ley: la Asignación Universal por Hijo y la Asignación por Embarazo del subsistema no contributivo; la asignación por hijo, la de hijo con discapacidad, la de maternidad y la prenatal del contributivo; la ayuda escolar anual, que cruza los dos porque el artículo 14 sexies se la da también a los titulares de la AUH; y la Asignación por Cuidado de Salud Integral, la única que exige haber tenido derecho a otra prestación para acceder. Son 118 reglas vigentes, 24 poblaciones, 16 plazos y 12 cuantías —once monetarias y una en especie—, cada pieza atada a su artículo y verificada contra el texto capturado por el cargador, más un control de esquema sobre las propias lecturas en `tests/unit/test_lecturas_curadas.py`. Falta curar el resto del corpus con texto —de la Ley 24.714 quedan seis prestaciones, casi todas pagos únicos por un hecho acreditado—: el trabajo es de lectura jurídica sobre cada norma, no de más ingesta._ |
